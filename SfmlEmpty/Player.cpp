...
void Player::update() {
	...
	// Interpolates the player's speed
	Player::lerp();
...
void Player::lerp() {
	...
	if (mCurrentCollisionB != 0 && mLastBlockToched == Terrain::BLOCK0ICY) {
		delta = sf::Vector2f(mIcyAcceleration.x * Toolbox::getFrameTime(), mIcyAcceleration.y * Toolbox::getFrameTime());
	}
	else {
	...
	// Interpolates the horizontal velocity up from stationary
	...
	// Interpolates the vertical velocity up from stationary
	if (differenceY > delta.y) {
		mVelocity.y += delta.y;
		lerpedY = true;
	}

	// Interpolates the horizontal velocity down to stationary
	if (mState == JUMPING || mState == FALLING) {
		if (differenceX < -airBorneDelta) {
			mVelocity.x += -airBorneDelta;
			lerpedX = true;
	...
	// Interpolates the vertical velocity down to stationary 
	if (differenceY < -delta.y) {
		mVelocity.y += -delta.y;
		lerpedY = true;
	}

	// If Max velocity has been reached in either direction
	if (!lerpedY)
...
void Player::updateState() {
	...
	if (mState != DEATH && mState != FALLDEATH) {
		if (mState == FALLING && mVelocity.y == 0) {
			std::cout << mClock.getElapsedTime().asMilliseconds() << std::endl;
			mState = LANDED;
